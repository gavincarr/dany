...
- `dany.go` (repo root) — package `dany`, the shared DNS query engine. Exposes `RunQuery` (typed-ANY aggregation, returns `([]Answer, []error)`), `Render` (turn `[]Answer` into the canonical tab-separated text), and `RunNXQuery` (NXDOMAIN probing), plus the `Query`, `Resolvers`, `Answer`, `QueryError` types and the `DefaultRRTypes` / `SupportedRRTypes` / `DNSSECRRTypes` / `NXTypes` / `SupportedUSDs` constants. Imported by the CLIs as `github.com/gavincarr/dany`.
...
- **DNSSEC split (`--all` vs `--dnssec`).** DNSSEC records are triaged by the RR-selection rule (low-frequency, non-opaque, human/agent-useful). `DNSKEY` and `DS` — the chain-of-trust summary answering "is this zone signed, how, and is the delegation secured" — are in `SupportedRRTypes`, so `--all` shows them. The opaque, constantly-re-signed signing records `NSEC`/`RRSIG` are **not** in `SupportedRRTypes` (kept out of `--all`); they live in a separate `DNSSECRRTypes` set (`DNSKEY,DS,NSEC,RRSIG`). `cmd/dany`'s `--dnssec` flag appends `DNSSECRRTypes` to whatever base set applies (default/`-t`/`--all`), deduped, in `parseOpts` *after* `WwwTypes` is fixed so www probes never inherit DNSSEC types. `DNSSECRRTypes` is also merged into `parseOpts`'s type-validation map, so an explicit `-t NSEC`/`-t RRSIG` stays valid even though they're absent from `--all`. (Querying bare `RRSIG` usually SERVFAILs — resolvers only emit RRSIGs alongside other RRsets — which is expected, not a bug.)
