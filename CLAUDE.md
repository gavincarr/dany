...
- `render_json.go` (repo root) — the JSON renderer. Exposes `BuildOutput` (assemble the typed `Output` envelope from `[]Answer` + `*Query` + `[]error`), `RenderJSON` (NDJSON-ready `\n`-terminated wrapper), the `Output` / `OutputAnswer` / `OutputError` envelope types, and one `*Data` struct per supported RR type. `SchemaVersion` is bumped only for breaking changes.
- `cmd/dany/main.go` — the `dany` CLI: simulates DNS `ANY` queries by firing the configured RR types (default `SOA,NS,A,AAAA,MX,TXT`) concurrently and aggregating results. `-f/--fmt {text,json}` selects the renderer; in JSON mode errors fold into the envelope instead of going to stderr.
...
- **Adding a new RR type** requires four coordinated edits: add to `SupportedRRTypes`; text path — add a `formatX(rrtype, *dns.X) string` helper and a `case *dns.X:` in `formatAnswer`'s dispatch (`dany.go`); JSON path — add an `XData` struct (snake_case json tags) and a `case *dns.X:` in `marshalData` (`render_json.go`). No `lookup()` change needed — it uses `dns.StringToType` to map the type name to the wire-format constant. Then add a golden test for text plus a row in the `TestBuildOutput_RRTypeDataShapes` table for JSON.
